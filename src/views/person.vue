          ...
          <el-tabs active-name="event" type="card" @tab-click="handle">
            <el-tab-pane label="用户动态" name="event">
              <div class="box4">
                <div>我的计划</div>
                <hr>
                <div v-for="(item,index) in AllPlanList" :key="index">
...
export default {
    ...
    data() {
        return {
            ...
            showQuitStudyRoom:false,//展示退出自习室的对话框
            studyRoomToQuit:{},//要退出的自习室
            AllPlanList:[],//用户创建的所有计划
        }
    },
    methods:{
      //选项卡跳转
    ...
    handleAvatarSuccess(res, file) {
      ...
      updateStudyRoomMessage(){
        var data={};
        data.studyRoomId=this.studyRoomToShow.studyRoomId;
        data.studyRoomName=this.studyRoomToShow.studyRoomName;
        data.studyRoomType=this.studyRoomToShow.studyRoomType;
        data.studyRoomIntroduction=this.studyRoomToShow.studyRoomIntroduction;
      ...
      //退出自习室
      SelectQuitStudyRoom(item){
        this.showQuitStudyRoom=true;
        this.studyRoomToQuit=item;
      },
      QuitStudyRoom(){
        var data={
          "studyRoomId":this.studyRoomToQuit.studyRoomId,
          "userId":this.User.userId
        }
        //发送请求
        this.$axios({
          url:"/studyRoom/leaveStudyRoom",
          method:"delete",
          data:data
        })
        .then(res=>{
          this.$message.success("退出自习室成功")
          this.showQuitStudyRoom = false;
          this.getStudyRoomWhichJoinedByUser();
        })
        .catch(res=>{
          this.$message.error("退出自习室失败"+res)
        })
      ...
      getAllPlan(){
        this.$axios({
          url:"/studyRoom/getAllPlanMakeByUser?userId="+this.User.userId,
          method:"get"
        })
        .then(res=>{
          this.AllPlanList=res.data.data;
        })
      }
